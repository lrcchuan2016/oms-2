...
<div class="container-fluid">
    ...
    <ol class="breadcrumb">
      ...
      <li class="active">渠道回收站</li>
    </ol>
    
    <div class="userListbox channel-list bg-white">
    	<form class="retrieval" action="channelAction_channelList?status=1" method="post" name="keyWordsForm">
    		<input type="hidden" name="struts.token.name" value="struts.token" />
			<input type="hidden" name="struts.token" value="3OZ1WJNLGUDRO58XE742UEH6HY6N74MU" />
             <div class="form-group pull-left" style="width:250px;">
              ...
              <a href="channelAction_channelList" class="btn btn-warning pull-right"><span class="glyphicon glyphicon-hand-left"></span>&nbsp;返回渠道列表</a>
              <!---->
    	</form>
    	...
    	<table class="table table-bordered table-hover" cellpadding="0px" cellspacing="0px" width="100%">
    		<thead>
    			<tr>
    				...
    				<td>logo</td>
    				<td>名称</td>
    				<td class="coverNum"><a href="channelAction_channelList?sortFiled=coverNum&&sortMod=desc&&status=1">覆盖用户</a></td>
    				<td class="installRate" ><a href="channelAction_channelList?sortFiled=installRate&&sortMod=desc&&status=1">安装率</a></td>
    				<td class="registerNum"><a  href="channelAction_channelList?sortFiled=registerNum&&sortMod=desc&&status=1">注册用户</a></td>
    				<td class="conversionRate"><a href="channelAction_channelList?sortFiled=conversionRate&&sortMod=desc&&status=1">转换率</a></td>
    				<td class="growthRate"><a href="channelAction_channelList?sortFiled=growthRate&&sortMod=desc&&status=1">增长率</a></td>
    		...
    		<tbody>
    			...
    			<tr id="${ch.id }">
    				...
    				<td>
    					<a class="btn btn-success btn-sm">恢复</a>
...
$(function(){
	...
	//还原
	$("td .btn-success").livequery(function(){
		$(this).click(function(){
			var id = $(this).parents("tr").attr("id")
			$.updateChannel(id);
		});
