            ...
            <div class="user-album-list clearfix albumID" id='<s:property value="albumID"/>'>
                <ul class="clearfix">
                 	<s:iterator value="paginationBean.records">
		                   <s:if test='media.audit_status=="2"'><li class="locked" id='<%="album" %><s:property value="media.id"/>'></s:if>
		                   <s:else><li id='<%="album" %><s:property value="media.id"/>'></s:else>
		                    	<a href="javascript:void(0);">
		                    		<s:if test='media.type=="0"'><img class="imgSrc" src='<s:property value="media.content"/>'></s:if>
		                    		<s:if test='media.type=="1"'><video controls width="160" height="160" src='<s:property value="media.content"/>'></video></s:if>
		                    	</a>
		                        <div class="info">
...
$.extend({
	...
	skip:function(){
		...
		var id = "${paramMap['id']}";
		var albumId = "${paramMap['albumId']}";
		//var option = "${paramMap['option']}";
		if(regex.test(page) && parseInt(page)<= parseInt(totalPage) && parseInt(page)>= 1){
			window.location.href="userAction_showAlbumPhoto?paramMap['id']="+id+"&paramMap['albumId']="+albumId+"&page="+page;
		}else{
