    ...
    <div class="row user-edit-box" style="padding-left:10px; padding-right:10px;">
       ...
       <div class="panel panel-default">
        ...
        <div class="user-content">
            ...
            <ol class="breadcrumb">
              <li><a href="userAction_showAlbum?paramMap['option']=${paramMap['option'] }&&paramMap['id']=${userBean.id }">
              	<s:if test="paramMap['option']==0">普通相册</s:if><s:elseif test="paramMap['option']==1">主题相册</s:elseif><s:elseif test="paramMap['option']==2">时间轴相册</s:elseif><s:else>全部相册</s:else></a></li>
              <li><a href="javascript:void(0)">${userBean.addr }</a></li>
            ...
            <div class="user-album-list clearfix albumID" id='<s:property value="albumID"/>'>
             <s:iterator value="paginationBean.records" var="am">
                <ul class="clearfix list-inline">
                 			<div>
                 				<h4><p class="utc font-weight">${am.utc }</p></h4>
                 				<span><b>标题：</b>${am.title }</span>
                 				<p class="text-overflow"><i>描述：</i>${am.detail }</p>
                 			</div>
                 			<c:forEach items="${am.mList }" var="m">
                 				<c:choose>
                 					<c:when test='${m.audit_status=="2"}'><li class="locked" id='<%="album" %>${m.id}'></c:when>
                 					<c:otherwise><li id='<%="album" %>${m.id}'></c:otherwise>
                 				</c:choose><a href="javascript:void(0)">
		                    			<c:choose>
		                    				<c:when test="${m.type==0}"><img class="imgSrc" src='${m.content }'></c:when>
		                    				<c:when test="${m.type==1}"><video controls width="160" height="160" src='${m.content }'>您的浏览器不支持HTML5 video标签</video></c:when>
		                    				<c:when test="${m.type==2}"><audio controls width="160"  src='${m.content }'>您的浏览器不支持HTML5 audio标签</audio></c:when>
		                    			</c:choose>
		                    		</a> 
		                    	</li>
                 			</c:forEach>
