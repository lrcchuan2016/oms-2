	    ...
	    <div class="col-xs-12 no-padder">
	    	<div class="panel panel-default no-padder">
				<div class="panel-heading">
					<h4>编辑媒资&nbsp;&nbsp;<span class="badge"><span class="glyphicon glyphicon-star"></span>号开头的为必填项. &nbsp;&nbsp;&nbsp;<span class="text-warning">[注]:</span>上传较大媒资封面可能耗时比较长，请耐心等待</span></h4>
				</div>
				<div class="panel-body">
					<form id="mediaForm" class="form-horizontal" action="javascript:void(null);">
						<input class="form-control hide" name="mediaList[0].timeLen" value="${mediaBean.timeLen }">
						<input class="form-control hide" name="mediaList[0].byteLen" value="${mediaBean.byteLen }">
						<input class="form-control hide" name="mediaList[0].width" value="${mediaBean.width }">
						<input class="form-control hide" name="mediaList[0].height" value="${mediaBean.height }">
						<input class="form-control hide" name="mediaList[0].createrId" value="${mediaBean.createrId }">
						<input class="form-control hide" name="mediaList[0].audit_status" value="${mediaBean.audit_status }">
						<input class="form-control hide" name="mediaList[0].createUtc" value="${mediaBean.createUtc }">
						<input class="form-control hide" name="commonMedias[0].id" value="${commonMedia.id }">
						<input class="form-control hide" name="commonMedias[0].publicLevel" value="${commonMedia.publicLevel }">
						<input class="form-control hide" name="commonMedias[0].userId" value="${commonMedia.userId }">
						<input class="form-control hide" name="commonMedias[0].utc" value="${commonMedia.utc }">
						<div class="row col-xs-8">
							<div class="col-xs-12">
								<div class="form-group">
									<label class=""><span class="glyphicon glyphicon-star"></span>存储路径</label>
									<input class="form-control" readonly value="${mediaBean.content }" name="mediaList[0].content" data-valid="[{rule:'not_empty'}]" type="text" placeholder="上传后文件相对路径 [不可更改]">
								</div>
							</div>
							<div class="col-xs-12">
								<div class="form-group">
									<label class=""><span class="glyphicon glyphicon-star"></span>媒资编码</label>
									<input class="form-control" type="text" readonly value="${mediaBean.id }" name="mediaList[0].id" maxlength="20" data-valid="[{rule:'not_empty'}]" placeholder="上传文件后自动获取到媒资编码">
								</div>
							</div>
							<div class="col-xs-12">
								<div class="form-group">
									<label class=""><span class="glyphicon glyphicon-star"></span>媒资名称</label>
									<input class="form-control" type="text" value="${commonMedia.title }" name="commonMedias[0].title" maxlength="20" data-valid="[{rule:'not_empty'}]" placeholder="填写媒资名称">
								</div>
							</div>
							<div class="col-xs-12">
								<div class="form-group">
									<label class=""><span class="glyphicon glyphicon-star"></span>媒资类型</label>
									<select name="mediaList[0].type" value="${mediaBean.type }" class="form-control">
							...
							<div class="col-xs-12">
								<div class="form-group">
									<label class="">媒资简介</label>
									<textarea class="form-control" value="${commonMedia.detail }" name="commonMedias[0].detail" rows="4" maxlength="100">${commonMedia.detail }</textarea>
								</div>
							</div>
						</div>
...
var W = window.top;
var mapObj = new HashMap(),formObj = null,actionObj = null,uploadProgressTimeout = -1,uploadTime = 0,resultKey = "",mediaType = "${mediaBean.type}";
var $uploadBtn = $(".mediaUpload").find("button.btn-primary"),$cancelUploadBtn = $(".mediaUpload").find("button.btn-danger");
var type = '${param.type }';
$(function(){
	...
	$(".subBtn").click(function(){
		if(formObj.validate_all()){
			var $editParam = $("#mediaForm").serialize();
			if(resultKey != "") $editParam += "&paramMap['mediaEdit']="+resultKey;
        	actionObj.submit({ 
        		param:$editParam,
        		successCallBack:function() {
...
function initUploadify(){
	...
	if(mediaType==0){
		...
		$(".getImageInfo").attr("src", "${mediaBean.thumbnail}");
	...
	$.post("json/media_findTypeList",function(data){
		if(data!=null&&data.list!=null){
			...
			$.each(data.list,function(){
				if(mediaType==this.id){
					html += "<option value="+this.id+" selected>"+this.description+"</option>";
				}else html += "<option value="+this.id+">"+this.description+"</option>";
			});
