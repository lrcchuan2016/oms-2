    ...
    <div class="userListbox channel-list" style="margin-right:20px;">
    	<ul class="nav nav-tabs">
          ...
          <li role="presentation"><a href="mediaPages_themeMusic">主题推荐音乐</a></li>
        	<li role="presentation" class="active"><a href="mediaPages_defaultMusic">默认音乐</a></li>
        </ul>
...
function htmlTemplate(index, data, recordIndex){
	...
	html += '<td><div class="btn-group"><button class="btn btn-warning btn-xs edit" id="'+data.mediaId+'"><span class="glyphicon glyphicon-edit"></span> 编辑</button></div></td>';
	...
	html += '<td><span class="glyphicon glyphicon-thumbs-down" style="cursor:pointer;"></span></td></tr>';
...
function dragUI(){
	...
	var sort = Sortable.create(container, {
	  ...
	  onUpdate: function (evt){
		...
		sortSubmit(param);
	    // $.post("json/media_sortMusic",{"paramMap['ids']":arrIds.join(","),"paramMap['flag']":flag},function(json){
...
function delAction($param){
 ...
 $(".glyphicon-thumbs-down").livequery(function(){
 	$(this).click(function(){
 		var tr = $(this).parents("tr");
 		var id = tr.attr("id");
		$.cancelRecommend(id,tr.children("td").eq(2).html());
 	});
 });
 
 $.extend({
	cancelRecommend : function(_id,name){
		W.$.confirm(
			"是否取消此默认音乐:"+name+"？",
			function(){
				//此处加入添加的AJAX
